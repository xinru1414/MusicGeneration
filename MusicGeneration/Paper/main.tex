...
We chose the music generation task as our final project for 10701. 

We plan to collect different styles of music as our input. Our goal is to 
generate music of different styles by using Variational Autoencoder (VAE).
According to \cite{47078}

VAE is known for its ability to reconstruct real data by learning from 
a latent space. It can represent high dimentional data in a low dimentional 
space. 

An autoencoder builds a lantent space of input dataset by learning to compress 
each input data into a vector of real numbers, and then reproduce the exact same
example from that vector. 
